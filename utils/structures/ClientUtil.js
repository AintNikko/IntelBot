...
class ClientUtil {
    ...
    compareStreaming(oldMember, newMember) {
        const s1 = oldMember.presence.activities[0] && oldMember.presence.activities[0].type === 'STREAMING';
        const s2 = newMember.presence.activities[0] && newMember.presence.activities[0].type === 'STREAMING';
        if (s1 === s2) { return 0; }
        if (s1) { return 1; }
        if (s2) { return 2; }
        return 0;
    ...
    async multiSend(channel, messages, delay) {
        await async.forEachSeries(messages, async m => {
